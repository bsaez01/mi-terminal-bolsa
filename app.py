...
def leer_portafolio():
    try:
        ...
        req = Request(f"{SHEET_URL}&cb={time.time()}", headers=headers)
...
try:
    ...
    if not df_h.empty:
        ...
        m_p_data = pd.DataFrame({'Mes': m_df.index.strftime('%b'), 'Año': m_df.index.year, 'Val': m_df.values}).tail(24)
        pivot_df = m_p_data.pivot(index='Año', columns='Mes', values='Val')
