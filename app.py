...
# --- 1. CONFIGURACIÓN ---
# ID corregido de tu hoja de Google
SHEET_ID = "1R9Jmk5Q781KcqJrkGkahojvGPnSKoYjX_LacmrrU6Wo"
# Esta es la URL que fuerza la descarga de los datos más recientes
...
def leer_portafolio():
    try:
        # Forzamos la descarga sin caché de Google
        ...
        # Si hay un error, lo mostramos en la barra lateral para saber qué falla
...
with st.sidebar:
    ...
    if not df_p.empty and 'ticker' in df_p.columns:
        ...
        if posiciones_pie:
            st.divider()
            df_pie = pd.DataFrame(posiciones_pie)
        ...
        st.info("Sincronizando o esperando datos...")
...
try:
    ...
    if not df_h.empty:
        ...
        with f2:
            ...
            st.write(f"PEG: {info.get('pegRatio', 'N/A')}")
        with f3:
            st.write("**Riesgo**")
        ...
        i1.info(f"**Predicción IA (5d):** {'ALCISTA' if pred > precio_act else 'BAJISTA'} (${pred:.2f})")
