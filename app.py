...
# --- 1. CONFIGURACIÓN ---
# Usamos el enlace de publicación directo que es el más estable
...
def leer_portafolio():
    try:
        # Forzamos la actualización sin romper la URL
        response = requests.get(SHEET_URL, timeout=10)
        if response.status_code == 200:
            df = pd.read_csv(io.StringIO(response.text))
            # Limpieza profunda de títulos y datos
...
try:
    ...
    if not df_h.empty:
        ...
        with f4:
            ...
            st.write(f"Precio Objetivo: ${info.get('targetMeanPrice', 'N/A')}")
    ...
    else: st.error("No hay datos históricos disponibles.")
