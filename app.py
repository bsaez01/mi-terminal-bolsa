...
# --- 2. FUNCIONES DE APOYO (CON CACHÉ PARA EVITAR ERRORES ROJOS) ---
@st.cache_data(ttl=600) # Guarda los datos de la bolsa 10 minutos
def obtener_datos_stock(ticker):
    try:
...
@st.cache_data(ttl=15) # Sincroniza la hoja cada 15 segundos
def leer_portafolio():
    try:
...
with st.sidebar:
    ...
    if not df_p.empty and 'ticker' in df_p.columns:
        ...
        st.info("Sincronizando datos de Google...")

# --- 4. LÓGICA DE MERCADO ---
ticker = st.session_state.ticker_seleccionado
stock, df_h = obtener_datos_stock(ticker)

if stock and df_h is not None:
    try:
        ...
        with f3:
            ...
            st.write(f"**Liq. Corriente:** {info.get('currentRatio', 'N/A')}")
        with f4:
