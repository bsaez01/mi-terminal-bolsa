...
# --- 1. CONFIGURACIÓN ---
# ID exacto que veo en tu captura de pantalla
SHEET_ID = "1R9Jmk5Q781KcqJrkGkahojvGPnSKoYjX_LacmrrU6Wo"
# Usamos el enlace de exportación directa que es el más rápido
...
@st.cache_data(ttl=5) # Sincroniza cada 5 segundos
def leer_portafolio():
    try:
        # Descarga directa sin complicaciones de URL
        response = requests.get(SHEET_URL, timeout=10)
        if response.status_code == 200:
            df = pd.read_csv(io.StringIO(response.text))
            # Normalizamos títulos a minúsculas y quitamos espacios
            df.columns = [str(c).strip().lower() for c in df.columns]
            
            # Limpiamos los datos numéricos (8 decimales)
            for col in ['cantidad', 'precio']:
                if col in df.columns:
                    df[col] = df[col].astype(str).str.replace('.', '', regex=False).str.replace(',', '.', regex=False)
                    df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)
            
            # Normalizamos Tickers y Tipo (compra -> Compra)
...
# --- 3. LÓGICA DE MERCADO (TODO RESTAURADO) ---
