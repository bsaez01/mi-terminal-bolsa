...
# --- 1. CONFIGURACIÓN ---
# Tu URL de publicación que ya sabemos que tiene los datos
...
def leer_portafolio():
    try:
        # Petición limpia al enlace de Google
        response = requests.get(SHEET_URL, timeout=10)
        if response.status_code == 200:
            # Leemos el CSV de la forma más estándar posible para evitar errores de motor
            df = pd.read_csv(io.StringIO(response.text))
            
            # Limpiamos nombres de columnas
            df.columns = [str(c).strip().lower() for c in df.columns]
            
            # Limpiador de números para asegurar los 8 decimales
            def limpiar_num(val):
...
with st.sidebar:
    ...
    if not df_p.empty and 'ticker' in df_p.columns:
        ...
        if posiciones_validas:
            ...
            st.plotly_chart(go.Figure(data=[go.Pie(labels=df_v['Ticker'], values=df_v['Cantidad'], hole=.4)]).update_layout(showlegend=False, height=180, margin=dict(t=0,b=0,l=0,r=0)), use_container_width=True)
    else:
