...
# --- 2. FUNCIONES DE DATOS (CORREGIDAS PARA EVITAR EL ERROR ROSA) ---
@st.cache_data(ttl=600)
def obtener_datos_limpios(ticker):
    try:
        stock = yf.Ticker(ticker)
        df_h = stock.history(period="2y")
        # Guardamos solo el diccionario de info, no el objeto stock completo
        info = stock.info 
...
with st.sidebar:
    ...
    if not df_p.empty and 'ticker' in df_p.columns:
        ...
        if posiciones_validas:
            st.divider()
            fig_p = go.Figure(data=[go.Pie(labels=[x['Ticker'] for x in posiciones_validas], values=[x['Cantidad'] for x in posiciones_validas], hole=.4)])
...
if df_h is not None and info is not None:
    try:
        ...
        pred = RandomForestRegressor(n_estimators=50).fit(X, y).predict(X.tail(1)).item()
    ...
    st.error("Esperando datos de mercado... Si este mensaje persiste, Yahoo Finance está limitando las peticiones. Espera 2 minutos.")