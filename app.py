...
def leer_portafolio():
    try:
        ...
        if response.status_code == 200:
            # Quitamos caracteres invisibles y detectamos el separador (coma o punto y coma)
            raw_text = response.text.strip().lstrip('\ufeff')
            # Intentamos leer con coma, si falla una sola columna, probamos con punto y coma
            ...
            # Limpiador de números ultra-flexible (detecta formato Chile 1.200,50)
            def limpiar_num(val):
...
# --- 2. INTERFAZ ---
st.set_page_config(page_title="Terminal Pro AI", layout="wide")
...
with st.sidebar:
    ...
    if not df_p.empty and 'ticker' in df_p.columns:
        ...
        if posiciones_validas:
            ...
            st.plotly_chart(go.Figure(data=[go.Pie(labels=df_v['Ticker'], values=df_v['Cantidad'], hole=.4)]), use_container_width=True)
    else:
        st.warning("No se detectan datos. Revisa que los títulos de la hoja sean: ticker, cantidad, precio, tipo")
...
else: st.error("Sin datos de mercado. Espera 1 minuto y refresca.")