...
# --- 1. CONFIGURACIÓN ---
# Tu URL de publicación que ya verificamos
...
def leer_portafolio():
    try:
        ...
        if response.status_code == 200:
            # Quitamos posibles caracteres raros al inicio (como el BOM de Excel)
            ...
            # Limpiador de números agresivo para Chile (comas a puntos)
            def limpiar_num(val):
        ...
        st.session_state['error_hoja'] = str(e)
        return pd.DataFrame()
...
with st.sidebar:
    ...
    if not df_p.empty and 'ticker' in df_p.columns:
        posiciones_validas = []
        # Agrupamos por ticker para calcular el saldo total
        for t in df_p['ticker'].unique():
            if str(t) in ['NAN', '', 'N/A']: continue
            ...
            if total > 0.00000001:
                ...
                if c2.button("Ver", key=f"btn_v_{t}"):
        ...
        if posiciones_validas:
            ...
            fig_p.update_layout(showlegend=False, height=180, margin=dict(t=0,b=0,l=0,r=0))
            st.plotly_chart(fig_p, use_container_width=True)
        else:
            st.info("Escribe tus datos en Google Sheets (revisa que diga 'Compra' o 'compra').")
    else:
        st.warning("No hay datos o las columnas en la hoja no coinciden (ticker, cantidad, precio, tipo).")
        if 'error_hoja' in st.session_state:
            st.caption(f"Error técnico: {st.session_state['error_hoja']}")

# --- 3. LÓGICA DE MERCADO (TODOS LOS DATOS INTEGRADOS) ---
...
if df_h is not None and info is not None:
    ...
    i2.warning(f"**Detector de Ballenas:** {'⚠️ ACTIVIDAD ALTA' if es_ballena else 'Normal'}")
    ...
    st.error("Esperando datos de mercado... Si no cargan, espera 1 minuto y refresca (límite de Yahoo Finance).")