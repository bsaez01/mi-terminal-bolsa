import os, requests, pandas as pd, numpy as np, certifi
...
# --- 1. CONFIGURACIÓN ---
# Asegúrate de que esta URL sea la correcta de tu hoja
...
def leer_portafolio():
    import time
    # Este truco obliga a Google a enviar los datos más recientes
    url_final = f"{SHEET_URL}&cache_buster={time.time()}"
    try:
        df = pd.read_csv(url_final)
        df.columns = df.columns.str.strip().str.lower()
        # Limpieza de números (comas y puntos)
