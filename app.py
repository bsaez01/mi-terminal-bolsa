    ...
    try:
        ...
        for col in ['cantidad', 'precio']:
            if col in df.columns:
                # Limpieza de comas chilenas y puntos de miles
    ...
    if not df_p.empty and 'ticker' in df_p.columns:
        for t in df_p['ticker'].unique():
            ...
            if total > 1e-9:
                ...
                # CORRECCIÓN VITAL: st.columns(2) con especificación
...
try:
    ...
    if not df_h.empty:
        ...
        i2.warning(f"**Detector de Ballenas:** {'⚠️ ACTIVIDAD INSTITUCIONAL' if es_ballena else 'Flujo Estable'}")
