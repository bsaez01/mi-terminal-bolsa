import os, requests, pandas as pd, numpy as np, certifi, io
...
# --- 1. CONFIGURACIÓN ---
# Usamos tu enlace de publicación directo que no falla
...
def leer_portafolio():
    try:
        # Lectura directa del enlace de publicación
        df = pd.read_csv(SHEET_URL)
        # Limpieza de columnas
        df.columns = [str(c).strip().lower() for c in df.columns]
        
        # Procesar números (comas a puntos)
    ...
    if not df_p.empty and 'ticker' in df_p.columns:
        for t in df_p['ticker'].unique():
            ...
            if total > 1e-9:
                ...
                col_txt.write(f"**{t}**: {total:.8f}")
                if col_btn.button("Ver", key=f"btn_{t}"):
...
# --- 3. LÓGICA PRINCIPAL (TODOS LOS DATOS) ---
...
try:
    ...
    if not df_h.empty:
        ...
        if st.button("📲 Notificar a Telegram"):
            enviar_alerta(f"🚨 Terminal AI: {ticker}\nPrecio: ${precio_act:.2f}\nPredicción: ${pred:.2f}")
            st.toast("Enviado")

    else: st.error("Sin datos para este símbolo.")
