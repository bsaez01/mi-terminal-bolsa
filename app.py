...
with st.sidebar:
    ...
    st.divider()
    st.subheader("📋 Mi Portafolio (Google)")
    df_p = leer_portafolio()
    
    # --- MODO DETECTIVE ---
    if not df_p.empty:
        # Si la columna 'ticker' no aparece, mostramos qué columnas encontró el programa
        if 'ticker' not in df_p.columns:
            st.warning(f"No encuentro la columna 'ticker'. Veo estas: {list(df_p.columns)}")
        else:
            posiciones_validas = []
